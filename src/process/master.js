...
export function listenWorker<M: mixed, R: mixed>(
  worker: AnyProcess,
  name: string,
  handler: Handler<M, R>
): Cancelable {
...
export function listenWorkerOnce<M: mixed>(
  worker: AnyProcess,
  name: string
): Promise<M> {
...
export function messageWorker<M: mixed, R: mixed>(
  worker: AnyProcess,
  name: string,
  message: M
): Promise<R> {
...
export function spawnProcess({
  ...
  function processOn<M: mixed, R: mixed>(
    name: string,
    handler: Handler<M, R>
  ): Cancelable {
  ...
  async function processSend<M: mixed, R: mixed>(
    name: string,
    message: M
  ): Promise<R> {
    await readyPromise;
  ...
  async function processImport<T: Object>(name: string): Promise<T> {
    ...
    if (!importCache[name]) {
      importCache[name] = messageWorker(
        worker,
        BUILTIN_MESSAGE.IMPORT,
        name
      ).then((childModule) => {
...
spawnProcess.import = async function importProcess<T: Object>(
  name: string
): Promise<T & {| __process__: SpawnedProcess |}> {
