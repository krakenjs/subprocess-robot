...
export function listen<M: mixed, R: mixed>(
  proc: AnyProcess,
  name: string,
  handler: Handler<M, R>
): Cancelable {
  ...
  if (nameRequestListeners[name]) {
    throw new Error(
      `Listener already registered for process with name: "${name}"`
    );
  }
...
export async function send<M: mixed, R: mixed>(
  proc: AnyProcess | NodeProcess,
  name: string,
  message: M
): Promise<R> {
  if (!proc) {
  ...
  proc.on("message", async (msg) => {
    ...
    if (type === MESSAGE_TYPE.REQUEST) {
      ...
      try {
        if (!handler) {
          throw new Error(
            `No handler found for message: ${name} in ${
        ...
        proc.send({
          ...
          uid,
          name,
          response: serializeObject(proc, response, listen),
        });
      } catch (err) {
        // $FlowFixMe
        proc.send({
      ...
      if (!responseHandler) {
        throw new Error(
          `No response handler found for message: ${name}, ${uid}`
        );
      }
